...
import renderer.filters.CannyFilter;
...
public class CannyEdgeDetector {
	...
	// methods
	nonmax nonMaxSuppressionObject;
	...
	public void process() {
		...
		// PixelGrabber grabber = new PixelGrabber(sourceImage, 0, 0, width,
		// height, orig, 0, width);
		// try {
		// grabber.grabPixels();
		...
		nonMaxSuppressionObject.init(orientation, width, height);
//		magnitude = nonMaxSuppressionObject.process();
		...
		for (int x = initX; x < maxX; x++) {
			for (int y = initY; y < maxY; y += width) {
				...
				: (tmp = Math.abs(yGrad * gradMag)) >= Math.abs(xGrad * seMag
					...
					orientation[index] = sector(Math.atan2(yGrad,
							xGrad));
		...
		for (int y = 0; y < height; y++) {
			for (int x = 0; x < width; x++) {
				if (data[offset] == 0 && magnitude[offset] >= high) {
					// if (data[offset] == 0 && orientation[offset] >= high) {
